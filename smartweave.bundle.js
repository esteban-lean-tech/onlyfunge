    ...
    1: [
      function (require, module, exports) {
        (() => {
            ...
            (function (globalObject) {
              ...
              function clone(configObject) {
                var div2,
                  convertBase,
                  parseNumeric,
                  P = (BigNumber3.prototype = {
                    constructor: BigNumber3,
                    toString: null,
                    valueOf: null,
                  }),
                  ONE = new BigNumber3(1),
                  DECIMAL_PLACES = 20,
                  ROUNDING_MODE = 4,
                  TO_EXP_NEG = -7,
                  TO_EXP_POS = 21,
                ...
                P.decimalPlaces = P.dp = function (dp, rm) {
                  ...
                  n =
                    ((v = c.length - 1) - bitFloor(this.e / LOG_BASE)) *
                    LOG_BASE;
                ...
                P.isGreaterThanOrEqualTo = P.gte = function (y, b) {
                  return (
                    (b = compare(this, new BigNumber3(y, b))) === 1 || b === 0
                  );
                };
                ...
                P.minus = function (y, b) {
                  ...
                  if (!xe || !ye) {
                    if (!xc || !yc)
                      return xc
                        ? ((y.s = -b), y)
                        : new BigNumber3(yc ? x : NaN);
                    if (!xc[0] || !yc[0]) {
                      return yc[0]
                        ? ((y.s = -b), y)
                        : new BigNumber3(
                            xc[0] ? x : ROUNDING_MODE == 3 ? -0 : 0
                          );
                    }
                  }
                  xe = bitFloor(xe);
                ...
                P.toFormat = function (dp, rm, format2) {
                  ...
                  if (format2 == null) {
                    ...
                    throw Error(
                      bignumberError + "Argument not an object: " + format2
                    );
                  }
                  str = x.toFixed(dp, rm);
                  if (x.c) {
                    var i,
                      ...
                      len2 = intDigits.length;
                    if (g2) (i = g1), (g1 = g2), (g2 = i), (len2 -= i);
                    if (g1 > 0 && len2 > 0) {
                      i = len2 % g1 || g1;
          ...
          async function simulateCreateContractFromSource(
            arweave,
            wallet,
            initState,
            contractSrc
            ...
            const deployInitStateTx = await simulateCreateContractFromTx(
              arweave,
              wallet,
              srcTx.id,
              initState
            ...
            srcTx.reward = (
              parseFloat(srcTx.reward) + parseFloat(initStateReward)
            ).toString();
            return srcTx;
          }
          async function simulateCreateContractFromTx(
            arweave,
            wallet,
            srcTxId,
            state,
            ...
            let contractTX = await arweave.createTransaction(
              { data: state },
              wallet
            );
          ...
          async function createContract(
            ...
            if (response.status === 200 || response.status === 208) {
              return await createContractFromTx(
                arweave,
                wallet,
                srcTx.id,
                initState
          ...
          async function createContractFromTx(
            arweave,
            wallet,
            srcTxId,
            state,
          ...
          function sub(...args) {
            return args
              .slice(1)
              .reduce((difference, operand) => difference - operand, args[0]);
          }
          function mul(...args) {
          ...
          function div(...args) {
            return Math.trunc(
              args
                .slice(1)
                .reduce((quotient, operand) => quotient / operand, args[0])
            );
          }
          function lt(a, b) {
            return a < b;
          ...
          function defaultTo(defaultValue, optionValue) {
            return optionValue !== null && optionValue !== void 0
              ? optionValue
              : defaultValue;
          }
          function err(value) {
          ...
          function fold(func, list2, initialValue) {
            if (list2 instanceof Array) {
              return list2.reduce(
                (accumulator, currentValue) => func(currentValue, accumulator),
                initialValue
              );
            }
            throw new TypeError();
          ...
          function isEq(...values) {
            if (
              values.length > 0 &&
              values.every((value) => typeof value === typeof values[0])
            ) {
          ...
          function log(arweave, ...str) {
            ...
            typeof arweave.getConfig().api.logger === "function"
          ...
          function normalizeContractSource(contractSrc) {
            contractSrc = contractSrc
              .replace(
                /export\s+async\s+function\s+handle/gmu,
  ...
  `;
          ...
          async function execute(handler, interaction, state) {
            try {
              ...
              throw new Error(
                `Unexpected result from contract: ${JSON.stringify(result)}`
              );
            } catch (err2) {
          ...
          async function readContract(
            ...
            const loadPromise = loadContract(arweave, contractId).catch(
              (err2) => {
                const error = new errors_default(
            ...
            const fetchTxPromise = fetchTransactions(
              arweave,
              contractId,
              height
            ).catch((err2) => err2);
            let [contractInfo, txInfos] = await Promise.all([
              loadPromise,
              fetchTxPromise,
            ]);
            ...
            for (const txInfo of txInfos) {
              ...
              if (result.type === "error") {
                ...
                log(
                  arweave,
                  `Executing of interaction: ${currentTx.id} returned error.`
                );
              }
              validity[currentTx.id] = result.type === "ok";
              state = result.state;
              const settings = state.settings
                ? new Map(state.settings)
                : new Map();
              ...
              if (evolve && /[a-z0-9_-]{43}/i.test(evolve) && canEvolve) {
                if (contractSrcTXID !== evolve) {
                  try {
                    contractInfo = await loadContract(
                      arweave,
                      contractId,
                      evolve
                    );
          ...
          async function addSortKey(arweave, txInfo) {
            ...
            const concatted = arweave.utils.concatBuffers([
              blockHashBytes,
              txIdBytes,
            ]);
      ...
      edges {
        ...
        cursor
          ...
          async function loadContract(arweave, contractID, contractSrcTXID) {
            ...
            const contractOwner = await arweave.wallets.ownerToAddress(
              contractTX.owner
            );
            contractSrcTXID =
              contractSrcTXID || getTag(contractTX, "Contract-Src");
          ...
          async function simulateInteractWrite(
            arweave,
            wallet,
            contractId,
            input,
          ...
          async function interactWriteDryRun(
            arweave,
            wallet,
            contractId,
            input,
            ...
            let { handler, swGlobal, contractSrcTXID } =
              contractInfoParam || (await loadContract(arweave, contractId));
            const latestState =
              myState || (await readContract(arweave, contractId));
            const from =
              fromParam || (await arweave.wallets.getAddress(wallet));
            const settings = latestState.settings
              ? new Map(latestState.settings)
              : new Map();
          ...
          async function interactWriteDryRunCustom(
            arweave,
            tx,
            contractId,
            input,
          ...
          async function interactRead(
            ...
            let { handler, swGlobal, contractSrcTXID } = await loadContract(
              arweave,
              contractId
            );
          ...
          async function createTx(
            ...
            if (!input) {
              throw new Error(
                `Input should be a truthy value: ${JSON.stringify(input)}`
              );
            }
            if (tags && tags.length) {
              for (const tag of tags) {
